...
class Robot(object):
    ...
    def __init__(self, room, speed):
        ...
        self.room = room
        self.speed = speed
        self.position = Position(random.random() * room.width, random.random() * room.height)
...
def runSimulation(num_robots, speed, width, height, min_coverage, num_trials,
    ...
    def doTrial():
        ...
        tRoom = RectangularRoom(width, height)
        robots = []
    
        #instantiates all the robots in a list
        for i in range(num_robots):
            robots.append(robot_type(tRoom, speed))
            
        time = 0
        roomSize = tRoom.getNumTiles()
        ratio = tRoom.getNumCleanedTiles() / roomSize
        ...
        while ratio < min_coverage:
            ...
            ratio = (tRoom.getNumCleanedTiles()*1.0)/roomSize

        return time
        
    for i in range(num_trials):
        times.append(doTrial())
    return (sum(times)*1.0)/num_trials
...
print  runSimulation(1, 1.0, 8, 8, 1, 30, StandardRobot)
